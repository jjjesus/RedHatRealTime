...
Continuous Deployment (CD) server (\textit{e.g.}, Jenkins) can install its own
copy of the kernel source code or kernel development tools and build a local
copy of the kernel, BSP, or device drivers (Figure \ref{fig:red_hat_dev}.  The
open-source Red Hat LiveMedia Creator package can be used for creating the
target Linux filesystem and the PXE boot tools.
...
\begin{figure}[H]
    \begin{center}
    \includegraphics[width=0.4\textwidth]{img/dev_sec_ops}
    \caption{DevSecOps Linux deployment in K3}
    \label{fig:dev_sec_ops}
    \end{center}
\end{figure}

The DevSecOps facility combines Red Hat RT Linux and target filesystem with the
KRFS application and the XES BSP and deploys directly to the bootflash and local
SSD of the SBC.
...
\hline
    ...
    Run-time License (yearly, per-instance) & 400 & 300 \\
\hline
\end{tabular}%
}
...
The RedHawk development license is for the Architect tool and requires
a dedicated machine.  For K3, three (3) RedHawk development licenses are needed.

The Red Hat RT subscription is a development licenses are per machine, planning
for three (3) development machines and one (1) CI/CD machine.

Assuming the Red Hat RT development subscription is not required for run-time
instances, the run-time cost for Red Hat RT is that for Red Hat Enterprise
Linux.
...
\subsubsection{Reusing Raytheon DevSecOps with Red Hat RT on XES SBC}
The DevSecOps infrastructure from another Raytheon group is based
on Red Hat RT running on a similar XES card.  This infrastructure can be
leveraged by KRFS when running Red Hat RT on the KRFS SBC.

\subsubsection{Leverage conventional builds for developers and CI/CD}
By eliminating the proprietary RedHawk Architect build requirements, individual
developers can turn through their build/test/deploy cycles more frequently and
using conventional build system can be integrated into CI/CD to enable frequent
and rapid deployment.

\subsubsection{DevSecOps with open-source Ansible and STIG Playbook}
The Raytheon DevSecOps infrastructure includes hardening tools based on
Ansible and an open source Ansible STIG playbook whose future
development and ongoing maintenance by the open-source community can be
leveraged by KRFS.

\subsubsection{Red Hat LiveMedia Creator}
The Red Hat LiveMedia Creator can be used to create PXE-boot images and DVD
installation images directly from source code by any developer or CI/CD machine.
The Raytheon DevSecOps includes tools and a process for using LiveMedia Creator
that can be leveraged for KRFS.
